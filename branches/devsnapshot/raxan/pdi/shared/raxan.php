...
class Raxan {
    ...
    public static function bindTemplate($rows, $options) {
        ...
        foreach($rows as $i=>$row) {
            ...
            // callback handler
            if ($fn) {
                 $rtn = ($callByVariable) ? $fn($row,$i,$t,$tplType,$fmt) : $fn[0]->{$fn[1]}($row,$i,$t,$tplType,$fmt);
                 if ($rtn===false) continue;         // skip row and continue
                 else if ($rtn!==null) $rt[] = $rtn; // check if a value was returned. if not let the binder handle the the insert
            }
            ...
            // format & sanitize row values
             foreach($row as $n=>$v) {
                ...
                if (!$fv) $row[$n] = htmlspecialchars($v);
                else {
                    // get name and style
                    $p = strrpos($n,' '); $s = '';
                    if ($p) {
                    ...
                    // set style
                    if ($s==='bold' && $f) $row[$n] = '<strong>'.$row[$n].'</strong>';
                    if ($s==='color') $row[$n] = '<span style="color:'.$fmt[$n.' color'].'">'.$row[$n].'</span>';;
                    if ($s==='style') $row[$n] = '<span style="'.$fmt[$n.' style'].'">'.$row[$n].'</span>';;
                }
            }

            $keys = !isset($keys) ? explode(',','{'.implode('},{',array_keys($row)).'},{ROWCOUNT},{ROWCLASS}') : $keys;
            $values = array_values($row);
            $values[] = $rc; $values[] = $cssClass[$tplType];
     ...
     * @param boolean $extrn Set to true if file will be loaded from path that's external to plugins.path
     * @return boolean
     */
    ...
    public static function loadUI($name) {
        if (!self::$isInit) self::init();
        $name = self::$config['raxan.path'].'ui/'.$name.'.php';
