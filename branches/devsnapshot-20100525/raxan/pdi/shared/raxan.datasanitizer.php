...
class RaxanDataSanitizer {
     ...
     * @return mixed Returns a date/time string value based on $format or null if value is not a valid date.  If input is an array then an array of date values is returned
     */
     ...
     * @return string  If input is an array then an array of email values is returned
     */
     ...
     * @return float Returns float if value is number or null if there was an error. If input is an array then an array of float values is returned
     */
     ...
     * @return int Returns interger if value is number or null if there was an error
     */
    ...
    public function isDate($key,$format = null) {
        try {
            if ($format===null) return $this->timestampVal($key) > 0 ? true : false;
                ...
                return strtolower($dt) === strtolower($this->formatDate($key,$format));
            }
        }
     ...
     * @return int Returns timestamp if value is a valid datetime string or null if error
     */
